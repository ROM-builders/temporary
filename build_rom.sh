...
git clone https://github.com/Miiyo/local-manifest.git --depth 1 -b aospa-topaz .repo/local_manifests
repo sync -c --no-clone-bundle --no-tags --optimized-fetch --prune --force-sync -j8

# build roms
source build/envsetup.sh
lunch aospa_akari_kddi-userdebug
export BUILD_USERNAME=exer
export BUILD_HOSTNAME=miyo
export TZ=Asia/Singapore #put before last build command
./rom-build.sh akari_kddi -c -t userdebug -v beta
