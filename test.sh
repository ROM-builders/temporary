...
if [[ $name_check == 0 ]]; then
	echo Please use init line url from rom manifest, its case sensitive. Also follow the format of build_rom.sh file of temporary repo main branch.
	exit 1
fi
...
if [[ $sed_check -gt 0 ]]; then
	echo Please dont use sed inside script, use local manifest for this purpose.
	exit 1
fi

tee_check=$(grep 'tee ' $CIRRUS_WORKING_DIR/build_rom.sh | wc -l)
if [[ $tee_check -gt 0 ]]; then
	echo Please dont use tee inside script, its not needed at all..
	exit 1
fi

clean_check=$(grep ' clean' $CIRRUS_WORKING_DIR/build_rom.sh | wc -l)
if [[ $clean_check -gt 0 ]]; then
...
if [[ $bliss_check -gt 0 ]]; then
	echo Please dont use make clean flag. Server does make installclean by default, which is enough for most of the cases.
	exit 1
fi

bliss_check=$(grep blissify $CIRRUS_WORKING_DIR/build_rom.sh | grep '\-d' | wc -l)
if [[ $bliss_check -gt 0 ]]; then
	echo Please dont use make installclean flag. Server does make installclean by default, which is enough for most of the cases.
	exit 1
fi

clobber_check=$(grep ' clobber' $CIRRUS_WORKING_DIR/build_rom.sh | wc -l)
if [[ $clobber_check -gt 0 ]]; then
...
if [[ $installclean_check -gt 0 ]]; then
...
if [[ $and_check2 -gt 0 ]]; then
	echo 'Please dont use & inside script.'
	exit 1
fi
...
if [[ $rclone_check != *$rclone_string* ]]; then
...
if [[ $pick_check -gt 0 ]]; then
	echo Please dont use repopick inside script, use local manifest for this purpose.
	exit 1
fi

cd_check=$(grep "cd *" $CIRRUS_WORKING_DIR/build_rom.sh | wc -l)
if [[ $cd_check -gt 0 ]]; then
...
if [[ $or_check -gt 0 ]]; then
	echo Please dont use or operator inside script
	exit 1
fi
...
if [[ $device == 'mi439' ]]; then
	echo "Please use device codename Mi439 also create your dt with this device code name."
	exit 1
fi
...
if [[ $BRANCH == *pull/* ]]; then
	...
	for id in 66806243 100027207 77049889 37245252 87101173 91236805 56505303 77262770 60956846 1133897 92011891 80823029 58514579 102499518 73420351 69832543; do
		logins+=" $(gh api -H "Accept: application/vnd.github+json" /user/$id -q '.login')"
	done

	for value in $logins; do
...
if [[ $CIRRUS_USER_PERMISSION == write ]]; then
	if [ -z "$CIRRUS_PR" ]; then true; else
		echo You are push user. Don\'t do pr and please follow pinned message in push group.
		exit 1
	fi
fi
